...
export default function AsmUserOrders({ userXid }) {
  ...
  const baseOrders = useMemo(() => {
    ...
    const branchFiltered = allOrders.filter(
      (o) => `${o?.userXid}` === `${userXid}`
    );
  ...
  const stats = useMemo(() => {
    ...
    const totalAmount = baseOrders.reduce(
      (sum, o) => sum + (Number(o?.sumOfInvoice) || 0),
      0
    );
    const completed = baseOrders.filter(
        ...
        <View style={styles.cardBody}>
          ...
          <View style={styles.rowItem}>
            ...
            <Text style={styles.amountValue}>
              ₹{Number(item?.sumOfInvoice || 0).toLocaleString()}
            </Text>
          </View>
        </View>
      </View>
